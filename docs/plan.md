...
**大概多久：** 先以 1 人約 13 週／2 人約 10–11 週作規劃目標；外部試用門檻和語言選定後，再調整排程。這不是已承諾的交期。

本節是首版範圍提案，取代前面較大的產品範圍；下列兩個取捨仍待你決定，合併文件不等於選定其中一條路。工程規則與票號對照移至 [詳細版](plan-detail.md#受邀試用版工程對照與待決策邊界)。
...
目前沒有新增拍板。選第二條時，要先調整工程票的放行依賴；選第一條則沿用現有完整 Alpha 門檻。
...
這是獨立的運行期限政策，不是閒置偵測；兩種選項都保留，尚未選定預設值。

語言另依 [PR #43 的 Runner 語言 ADR](https://github.com/our-sandbox-agent/sandbox-console/pull/43) 定案：不預設 Node，也不預排重寫。下一步先確認這兩個取捨與語言維護負責人，再按詳細版拆實作驗收。
