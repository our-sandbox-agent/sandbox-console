...
> 語言已定案（2026-09-21）：Runner 與 CLI 用 Go，控制平面 TypeScript，前端維持 JavaScript，不安排改寫。見 [Runner 語言 ADR](adr/runner-language.md)。
...
## 受邀試用版：收斂後的範圍（2026-09-21）

**做完會看到：** 一行啟動 Claude，交代任務後關掉筆電，再連回來取回成果；暫停後檔案與對話紀錄保留。
**先做哪些：** 受邀試用、API key、基本下載、冷 Suspend 與用量紀錄；正式收費、記憶體快照和 Firecracker 往後放。
**大概多久：** 先以 1 人約 13 週／2 人約 10–11 週作規劃目標；受限試用的放行票拆完後再校準。這不是已承諾的交期。

本節是首版範圍，取代前面較大的產品範圍。下列兩個取捨已於 2026-09-21 由創辦人決定，保留兩條路的比較作為紀錄。工程規則與票號對照移至 [詳細版](plan-detail.md#受邀試用版工程對照與待決策邊界)。

**五個方向**

- 找有長任務或多 session 需求的受邀開發者，首版只做 Claude、使用者自帶 API key。
- 用 repo clone 帶入專案；成果可自己 git push，也能下載。沒有 push 權限時仍能取回檔案。
- 冷 Suspend 保存工作檔案與選定的家目錄資料，用明確 session ID 接回對話；重新啟動程序，不承諾回到同一畫面。
- 自動 Idle 保留，但要保護忙碌任務；有可靠完成訊號才自動 Suspend。限速後 CPU 需求回升或開始新任務時，要能恢復 Active。
- 試用先不收費，用量從第一天記錄。憑證的保存與備份分開處理，不把登入資料整包備走。

**決定一：先邀 3 到 5 人做受限試用，再補齊完整 Alpha 門檻**

| 選項 | 好處 | 代價與前提 |
|---|---|---|
| 完成完整 Alpha 門檻後再邀 | key 代理、出口限制與其他放行驗收一起完成，對外邊界較清楚。 | 第一批回饋較晚；多花多久要按缺少的工作估算，3–4 週目前只是討論估計。 |
| 先邀 3–5 人作受限試用，再補齊完整門檻 | 更早驗證是否有人需要產品。 | 需另定這批人的隔離、憑證、成本與事故處理底線，明確列出延後的保護及風險；不能僅靠「信得過」就省略驗收。 |

已選第二條。接下來要另開「受限試用放行」票，列出這批人的隔離、憑證、內網／metadata 阻擋、容量、備份還原與事故處理的具體底線，以及哪些保護延後、風險怎麼向受邀者揭露；完整 Alpha（#24）仍是擴大邀請前的門檻。

**決定二：不設預設最長運行時間，由使用者自行啟用**

| 選項 | 好處 | 代價與前提 |
|---|---|---|
| 平台提供可延長的預設上限 | 對卡住或沒有 hook 的沙盒有時間上的成本上限。 | 到期可能中斷長任務；須在建立時明示、提前提醒，並決定上限時數與延長規則。 |
| 不設預設時限，使用者自行啟用 | 長任務較不會因期限被中斷。 | 無完成訊號的沙盒可能長期停在 Idle 佔用記憶體；要靠容量、配額與告警管理成本。 |

已選第二條。平台不預設時限；使用者可在建立時或之後自行設定上限，到期前提醒、可延長，到期冷 Suspend 並明示會中斷執行中的工作。成本改靠每人配額、主機容量與告警控制，這三項的數字在第 2 步與第 4 步 A 段定。

語言已定案：Runner 與 CLI 用 Go，見 [Runner 語言 ADR](adr/runner-language.md)。下一步是拆受限試用的放行票，然後按詳細版開第 1 步。
