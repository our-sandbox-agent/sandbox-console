...
> 從現在的瀏覽器雛形，到「一行 `sandbox claude` 真的能跑、能暫停接續、能收費」，拆成 8 步。每步的估時是 **1 位工程師** 的週數；2 人時第 1、2 步和第 5、6 步可以並行。每步的具體做法、指令、驗收與風險在 [plan-detail.md](plan-detail.md)。**2026-09-21 起以「受邀試用版」為第一個目標，範圍與順序見最後一節。**
...
## 受邀試用版：五個方向決定後的調整（2026-09-21）

五個方向性建議全部採納，但各調整一點；以下是採納後的規則與對計劃的改動。

| 決定 | 採納方式 | 對計劃的改動 |
|---|---|---|
| 1. 第一批：受邀個人開發者 | 同意。挑「有痛」的人（三選二）：已用 `--dangerously-skip-permissions` 或 Docker 跑 Claude Code；習慣同時跑 2 個以上 session；常有 30 分鐘以上的長任務想關電腦走人。硬條件：自己有 Anthropic API key。5 到 10 人，先 30 分鐘訪談再給邀請。 | 第 2 步的 GitHub 登入加名單（不在名單回 403，約半天）。映像檔與 CLI 只做 Claude，Codex／Harness 拿掉（省約 1 天、映像檔少 100 MB）。租戶隔離不因人少而砍。 |
| 2. 第一版價值 | 同意，但驗收要加一件本機做不到的事：交代 30 分鐘以上的任務 → 關終端或筆電離開 1 小時 → `sandbox connect` 看到已完成 → 成果已取回。第二案：同時開 2 個沙盒互不干擾。 | 取回成果兩條路都便宜：沙盒內自己 `git push`（需要家目錄持久化，見第 3 題）；CLI 加只下載的 `sandbox cp <id>:/workspace ./out`（約 1 到 2 天）。續傳上傳、Files 下載、Snapshot／Fork 整個第 5 步延到試用後。 |
| 3. Suspend 只留檔案 | 同意，但「檔案」要包含家目錄：`~/.claude` 對話紀錄、gh 登入、shell 歷史。Resume 時用 `claude --continue` 接回同一段對話，就有八成「程序接續」的體感。對外只講「Suspend 不收運算費、恢復後檔案與對話都在」，不講秒數、不講「同一畫面」。 | 第 1 步多掛一顆 volume 到 `/home/agent`（約半天）。試用期每晚把 volume 打包備份到 bucket。第 4 步 B 段與第 8 步 B 段改成「後續研究」，不排進里程碑。第 4 步從 5 週縮到約 2 週。Runner 語言依 [language-proposal.md](language-proposal.md) 定案後直接以該語言實作，不預設 Node、也不預排改寫。 |
| 4. 何時自動暫停 | 原則同意（沒敲鍵盤不等於閒置），但不能變成「沒有自動降級」，那是我們唯一的差異化，也是不收費期間主機成本唯一的煞車。改成三層規則，見下方。 | 第 4 步 A 段照做，狀態表改兩格；驗收加一案「從未收到 hook 的沙盒放一整天只會 Idle、不會 Suspend」。第 4 步上線前，第 1 步的開發期回收器改成「running 超過 24 小時就冷 Suspend」當硬天花板，不刪。 |
| 5. 自帶 key、先不收費 | 同意。受邀者開一把專用 key，並在 Anthropic Console 自己設每月上限（例如 US$20）；邀請信明講「目前 key 在沙盒內可讀、出口未限制，第 6 步後改為代理注入」。我方不存明文 key：CLI 每次從本機環境變數重送。 | 第 7 步整步延到試用後，只抽兩塊進來：最小告警（服務掛了寄 email，約 2 天）與各狀態秒數的 Usage 頁（不算錢，但顯示「若照 E2B 全時計費你會付多少」，讓差異看得見，約 2 天）。配額改成每人同時 running 1、總數 3，每台主機上限依規格定（16 GB 約 3 到 4 個沙盒），受邀人數反推。 |

**自動降級的三層規則**

1. **Active → Idle 永遠自動**：無鍵盤且 CPU 安靜 60 秒，或 Claude Code 回報完成後 30 秒。Idle 只是限速，不殺程序，打一個字就回來。
2. **Idle → Suspend 只認正向訊號**：Claude Code 的完成 hook 比忙碌 hook 新、且無鍵盤 N 分鐘，才自動冷 Suspend；或無鍵盤且 CPU 安靜連續 12 小時的硬天花板。忙碌超過 2 小時改成提醒「要手動 Suspend 嗎」，不強制。
3. **拿不到訊號就不自動 Suspend**：沒有 hook 的 agent、或使用者改掉 hook，只會停在 Idle。手動 Suspend 從第 1 步就有，定位是輔助。

**受邀試用版的範圍與估時**

第 1 步（2 週，用 ttyd 取代自寫的 PTY 串流）→ 第 2 步（3 週，含邀請名單）→ 第 3 步 A 段（4 週，含 `sandbox cp` 下載，不含語言改寫）→ 第 4 步 A 段（2 週，含三層規則、最小告警、秒數版 Usage）。1 人約 11 週，含 15% 緩衝約 13 週；2 人第 1、2 步並行約 10 到 11 週。原計劃要到第 20.5 週的 M2 才有同等內容。

**還有兩個決定要拍板，會直接影響順序**

1. **受邀者什麼時候進來**：第 3 步做完（約第 9 到 11 週）就邀，接受並揭露「key 在沙盒內可讀」；還是先做第 6 步 A 段（代理注入 key、出口白名單，約 2 週）再邀。建議：先邀 3 到 5 個信得過的人，第 6 步 A 段做完再擴大到 10 人。私有 repo 的憑證在第 6 步前由使用者自己在沙盒內 `gh auth login`，家目錄持久化後不會掉。
2. **API key 還是 Claude 訂閱**：個人開發者很多用 Pro／Max 訂閱而非 API key。訂閱憑證放在 `~/.claude`，第 3 步的免問答設定與第 6 步的代理換 key 都要不同設計，必須在第 3 步開工前決定。建議：第一版只收 API key，邀請條件寫明；若候選人有一半因此拒絕，再回頭做訂閱登入。
