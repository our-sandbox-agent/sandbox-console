...
### 記憶體終止補充（#67 提案，待創辦人接受與 Runner 實作驗收）

記憶體上限可能終止整個沙盒，包含 Agent、tmux 與執行中的工作。低於上限的單程序限制不能保證整個沙盒都能存活。已落盤的指定 workspace／home 資料會保留；尚在程式緩衝區、未完成的寫入與記憶體狀態可能遺失。終止後必須顯示明確原因，預設由使用者操作重新啟動，不靜默重跑工作或呼叫付費模型。成功重啟不代表舊 tmux 或 Claude 程序已復原；Claude session ID 接回仍待實測。連續重啟失敗需停止重試並通知維運。

上述是擬採用的試用契約，尚未實作；不得在功能驗收前宣稱平台已提供。完整偵測、通知、檔案保存與重啟條件見 [記憶體恢復提案](../adr/memory-session-recovery.md)。
