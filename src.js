...
setInterval(()=>{tick(view!=='blog');save();document.querySelectorAll('[data-cost]').forEach(el=>{const b=boxes.find(x=>x.id===el.dataset.cost);el.textContent=money(cost(b));});const total=document.querySelector('#total-cost');if(total)total.textContent=money(boxes.reduce((a,b)=>a+cost(b),0));},5000);
